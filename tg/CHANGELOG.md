...
- **Другий мерж дня: `sitemap.xml` для індексації — `dev` → `main` `60317fb` (`--no-ff`, за
  явним «так» власника).** Новий файл у корені сайту, статичний, 65 адрес у канонічній формі
  без `.html`: `/`, `/architect`, `/claude-code`, `/roadmap`, `/verify`, три довідники
  `claude-code-ref-*`, 12 + 22 + 23 уроки. `/certificate` свідомо не включений (особиста
  сторінка за входом). `lastmod` — дата останнього коміту файла (лендінги — дата їхнього
  конфіга, `2026-09-05`). Разом поїхали лише документи після релізу 005 (`CLAUDE.md`,
  журнали `dev/build`, цей файл) — жоден із них не роздається. `.assetsignore` не чіпався;
  `robots.txt` не створювався. **Прод звірено:** `/sitemap.xml` 200 `application/xml`
  7472 Б, побайтово = репозиторій; усі 65 адрес на проді 200; `CLAUDE.md` 404. Аліас
  `ai-academy.andriy-puhalsky.workers.dev` перші секунди віддавав 404 на новий файл — відставання
  поширення, за хвилину 200. SQL, Edge Functions, секрети — не чіпались. Подання в Google
  Search Console — власник вручну.
...
- **Застосовано `005-3-cleanup-qa-certificate.sql`** — з `public.certificates` прибрано
  ...
  сесії через Supabase MCP за прямим разовим дозволом власника** («зроби самостійно як
  виключення», 2026-09-05) — виняток із правила «зміни бази тільки файлами на
  затвердження». Файл:
  `dev/build/005-ai-terminal/02-backend/db/005-3-cleanup-qa-certificate.sql`.
- **Як саме:** DO-блок із запобіжником — спершу `count(*)` під умовою (ім'я **і** доба
  видачі), і якщо це не рівно один рядок, транзакція падає з винятком, нічого не
  видаливши. Перед видаленням рядок прочитаний повністю, його значення передані власнику
  **поза репозиторієм** (публічний гіт: `public_code` і `user_id` у нього не кладуться) —
  без них точне відновлення неможливе, бо `public_code` генерується випадково.
- **Стан після:** `certificates` — **0 рядків**, як і було до QA-прогону; це знову
  таблиця, у якій за всю історію проєкту не лежить жодного документа. `progress`,
  `quiz_attempts`, `enrollments`, `profiles` тестового акаунта **не чіпались** — власник
  просив прибрати саме сертифікат. Наслідок, про який треба знати: прогрес того акаунта
  лишається 23/23, тож повторне позначення останнього модуля завершеним видасть **новий**
  сертифікат з **іншим** кодом. Схема, політики, гранти, функції не змінювались; Edge
  Function і Cloudflare не чіпались.
