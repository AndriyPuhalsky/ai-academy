...
- **Статус:** **в роботі — контент: j01–j22 написані й відрецензовані, хвости закриті; хвиля 5 (j19–j22) закрита повністю 2026-09-19 (день). Далі — іспит j23 і три довідники, кожне за словом власника. Фірма сюжету з 2026-09-19 — «Марципан» (рішення власника; була «Кориця»).** (Каркас до дизайну готовий з 2026-09-17.)
  ...
  **Хвиля 1 пройшла повністю 2026-09-17 (автори ×3 → рецензент ×3):** `modules/jira-01…03.html` фінальні
  (3× «готово з зауваженнями», чекер 0 ✗), звіти `reports/jNN.md` + `jNN-review.md`; знімки 22–24 дозняті,
  ⚠ №3 фактів закрито. **Хвиля 2 (ніч 2026-09-17 → 18): автори ×7 на j04–j10 — усі 7 готові**, чекер 0 ✗,
  «найдовша» 0–2 з 6, коміти `496ff0c`…`52f85df`; §8 звітів → `cross-findings.md`, уточнення карток →
  `program.md` «Уточнення після хвилі 2», ⚠ → `facts-free-plan.md`. **Рішення власника (ніч):** агенти пишуть
  лише контент; хвилі 3 (j11–j14), 4 (j15–j18), 5 (j19–j23) і довідники затверджені наперед; рецензенти на
  кожну партію. **Рецензія хвилі 2 (2026-09-18, ранок, після двох зривів за лімітом):** 7× «готово з зауваженнями», 72 правки
  (коміти `d77f0e5`…`71cd305`), знахідки → `cross-findings.md` «Від рецензентів хвилі 2». **Хвиля 3 (2026-09-18, день): автори ×4 на j11–j14 — усі 4 готові** (коміти `8fd7fe6` j12 · `c6515d5` j14 ·
  `fd6087c` j13 · `0f0e880` j11), чекер 0 ✗, §8 → `cross-findings.md` «Від авторів хвилі 3», `program.md`
  «Уточнення» п. 12 + `WEB-4`/`WEB-5` у §3. **Рецензія хвилі 3 (2026-09-18, день): 4× «готово з зауваженнями», 49 правок** (коміти `268adde` j13 · `102cdb3`
  j12 · `76163f3` j11 · `4687992` j14), знахідки → `cross-findings.md` «Від рецензентів хвилі 3», `program.md` п. 13.
  **Хвиля 4 (2026-09-18, ніч; слово власника «запускай четверту хвилю»): автори ×4 на j15–j18 — усі 4 готові** (коміти `295177c` j15 ·
  `fede0ab` j18 · `d712910` j16 · `8c887d3` j17), чекер 0 ✗, квізи найдовша 0/6, маркерів 0 (новий `check-quiz-markers.py`),
  234 цитати посимвольно; §8 → `cross-findings.md` «Від авторів хвилі 4», `program.md` §3 + «Уточнення» п. 15,
  `facts-free-plan.md` «Дописано після хвилі 4». **Рецензія хвилі 4 (2026-09-19, ніч): 4× «готово з зауваженнями», 36 правок
  рецензентів + 12 правок кореневої сесії, 224 цитати = 0 розбіжностей** (коміти `ea376fd` j15 · `4346c3c` j16 · `7639657` j17 ·
  `243ee8f` j18 · `be0e202` хвости в j01/j09); знахідки → `cross-findings.md` «Від рецензентів хвилі 4», `program.md` п. 16 і
  «Відкриті питання власнику» п. 7–9. Чекер 0 ✗ і маркерів 0 **на всіх 18 уроках**.
  **j01–j18 написані й відрецензовані. Далі:** хвиля 5 — автори j19–j22 → рецензенти → іспит j23 (рекомендація: після рецензії
  j19–j22) → довідники ×3 — **за словом власника**; чеклист і рядки «особливе» для j19–j23 — `dev/build/JOURNAL.md` «Старт наступної сесії»
- **Заведено:** 2026-09-17
...
2. **Сюжет — одна невелика фірма з відділами** (робоча назва — кав'ярня-пекарня «Марципан», ~8 людей):
...
11. **Сюжет «Марципан» і 8 ролей — запропоновані сесією, власник править** (розділ 3 програми, одна
...
7. **Реліз:** версія курсу `1.0.0`, трьом іншим патч (дропдаун); `site.updated` ×4; `roadmap.json`
   ...
   **Чеклист релізу, рядок з 2026-09-17:** уроки j01–j03 (і всі наступні) посилаються на `../jira.html`
   (шапка, футер, «До програми», `#map`), а файла до `дизайн готовий` немає — на dev-превʼю «На головну» дає 404.
   Перед мержем QA перевіряє, що **всі внутрішні посилання уроків і довідників → 200** (`curl -sL`).
...
- назва фірми-сюжету і ~8 людей з ролями («Марципан»?);
