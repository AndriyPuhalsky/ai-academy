...
- **Статус:** **в роботі — контент: j01–j22 + іспит j23 написані й відрецензовані; хвиля 6 (іспит j23 + хвости попередніх хвиль + реєстр `open-claims.md`) закрита 2026-09-20. **2026-09-24 — сесія хвостів із живим sandbox** (`ca3576b`, `00cc867`, `f170c00`): закрито все, що не чекає JSM/Confluence/ділового спейсу; спростовано тезу «AI-кнопки на Free — лише апсел»; час читання 190 сл/хв застосовано. Далі — три довідники, за словом власника. Фірма сюжету з 2026-09-19 — «Марципан» (рішення власника; була «Кориця»).** (Каркас до дизайну готовий з 2026-09-17.)
  ...
  **Хвиля 6 (2026-09-20; слово власника «продовжуй наступну хвилю, і закрий хвости з попередніх якщо вони є»):** автор іспиту
  `j23` (`15fff2d`; 26 питань з опорою в чинних текстах, ≈ 610 тис. токенів, 38 хв) → рецензент j23 (`e2f78d0`; «готово з
  зауваженнями», 22 правки + 5 правок кореневої сесії, ≈ 345 тис., 32 хв); паралельно рецензент зібрав **реєстр незакритих
  тверджень без джерела** — `01-authoring/open-claims.md` (113 записів; ⛔ 1 і пʼять із шести 🔴 закриті того ж дня, плюс
  чотирнадцять 🟡; відкрита 🔴 — «на Free доступні обидва типи спейсу», три уроки, чекає слова власника або sandbox); коренева
  сесія закрила хвости текстів: квіз j20 під теми картки, дублі після імпорту в j21, Confluence і сертифікації в j22, 🔴
  «одинадцяту людину Jira додати не дасть» у j02/j20 (суперечило довідці про рахунки), три розбіжності між уроками зі звіту
  автора іспиту. Три скрипти на кінець дня — 0 ✗ і 0 маркерів на 23 сторінках. Нові інструменти в `01-authoring/`:
  `reading-time.py`, `ref-promises.py` (+ `ref-promises.md`), `dump-lessons.py`, `docs-text.py`, `cdp-text.mjs`. Документи:
  `cross-findings.md` (чотири розділи від 2026-09-20), `program.md` «Уточнення» п. 19, контракт §3 «Скелет іспиту», шаблони
  промптів — блок «Додано після хвилі 6». Вхід наступної сесії — `dev/build/JOURNAL.md` «Старт наступної сесії».
- **Заведено:** 2026-09-17
...
7. **Реліз:** версія курсу `1.0.0`, трьом іншим патч (дропдаун); `site.updated` ×4; `roadmap.json`
   ...
   **Чеклист релізу, рядки з 2026-09-18…19 (зведено з хвиль 2–5, щоб PM і QA не шукали по звітах):**
   (а) **рішення власника 2026-09-19:** монограма сертифіката — **`AIJ`** (рядок `BRANDS` у `js/certificate.js`); пункт меню
   «Курси» — **«Jira з нуля»** (чотири лендінги, десктопне меню й мобільна шторка); блок «Хто веде» (`author` у
   `jira.config.json`) — **заглушка**: секцію не публікувати з порожнім текстом і порожнім `href`, доки немає сторінки «Про
   засновника» (посилання піде на неї, не на профіль Atlassian Community); фірма сюжету — **«Марципан»** (у hero лендінга
   картка `MY-1` — звірити назву в макеті); (б) `js/mermaid-theme.js` — `useMaxWidth: false` для `state`: j10 і j19 зі
   `stateDiagram-v2` інакше стискаються замість прокрутки (`sequence` у темі вже є — j22 не стискається, але широка);
   (в) **заміри на 390 px:** `win__table` на 4–7 колонок (j07, j08, j14, j20 W3 із заголовком `Role - Jira - marcypan`),
   перший багаторядковий `term` (j15), три `term` j22 з українським запитом одним рядком до ~210 знаків (різати переносом не
   можна — рядок копіюють кнопкою), `sequenceDiagram` j22 з чотирма учасниками, `ds-tbl` груп j20 (довгі `<code>` у другій
   колонці), чеклист j21 (таблиця 20 × 3); (г) `win__topbar` — 0 вживань у 22 уроках: дизайн зводить до одного рішення, білд
   робить sed-пас; `win__placeholder` ужитий не за призначенням у j14, j16, j17, j19, j20 (підзаголовок секції, рядок пунктів
   меню) — замінити на клас, який дасть дизайн; реєстр заявок дизайну — `cross-findings.md`, пункти «Дизайну» в наскрізних
   розділах хвиль 2–5; (д) **перед релізом — повторна посимвольна звірка цитат усіх 22 уроків і трьох довідників з живою
   довідкою** (Atlassian переписує сторінки на ходу: цитата j05 зникла за добу) і перевірка посилань j22 після перезапуску
   форумів спільноти 2026-09-22; (е) `modules/jira-23.html` і три `jira-ref-*.html` мають існувати до QA — уроки посилаються
   на іспит і довідники за назвою; (є) час читання в шапках уроків — звести одним правилом слів/хв: **скрипт готовий з
   2026-09-20** — `01-authoring/reading-time.py` (звіт і `--apply`); автори ставили від 137 до 218 сл/хв, найближче до
   чинних значень правило **190 сл/хв** (сума зсувів 43 хв на 22 уроках, пʼять уроків зсуваються на 3+ хв: j02 25 → 19,
   j03 25 → 18, j01 20 → 17, j09 22 → 19, j17 22 → 25); **швидкість — рішення власника, нічого не застосовано**; застосовувати
   після останніх правок текстів (дозйомка sandbox ще змінить j16, j18, j19).
   **Чеклист релізу, рядки з 2026-09-20 (іспит j23 і реєстр `open-claims.md`):** (ж) **`modules/jira-23.html` — довідники
   названі текстом без `href`** (файлів ще не було): коли `jira-ref-*.html` зʼявляться, поставити посилання в блоці 8 («Три
   довідники — у закладки») і в ЧаПи «Чи можна користуватись довідниками під час іспиту?» — так зроблено в
   `claude-code-23.html`; (з) **гілка `isFinalClick()` у `js/module.js` (останній модуль → діалог підтвердження імені →
   `submit_quiz` → видача сертифіката) спрацьовує лише на сторінці іспиту** — після застосування міграції `012-1` QA проходить
   її живим акаунтом до сертифіката й `/verify`; без міграції перше, що побачить учень, — «невідомий модуль» у консолі;
   (и) заміри іспиту на 390 px: друга `.ds-tbl` (розподіл питань, 4 колонки); назви блоків у сайдбарі до 37 знаків — на
   280 px; три Mermaid-скрипти на сторінці без діаграм (каркас, як у `claude-code-23.html` — рішення білду, чи прибирати);
   `#completeTitle` на іспиті перезаписується на «Модуль позаду?» — точка Б-14 тут найпомітніша; (і) **перед релізом пройти
   `01-authoring/open-claims.md`**: розділ «Стан після проходу кореневої сесії» → усе з позначкою sandbox і 🟡, що лишились;
   розділ 8 — чотири сторінки довідки, що змінились під курсом (Rovo потребує підтвердженого бізнес-домену; список фільтрів у
   бічній панелі team-managed спейсу; `create-a-new-project` переписана; адреса сайту «3 проти 15»); цей прохід робити разом
   із рядком (д) — повторною посимвольною звіркою цитат; (ї) публічні сторінки, які малює браузер (сертифікації, спільнота
   після перезапуску форумів 2026-09-22), знімати `01-authoring/cdp-text.mjs` — розширення Chrome для них не потрібне.
