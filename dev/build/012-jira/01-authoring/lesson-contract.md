...
| 10 | картка завершення `#completeTitle` | «Модуль позаду?» | **лишити як у базі.** `js/module.js` після завантаження перезаписує цей текст на «Модуль позаду?» / «Модуль пройдено!» — це спільний код, правити його може лише білд (точка Б-14 в `inventory.md`/`task.md`: слово з `site.moduleWord` або лишити «модуль»). Автор j01 замінив на «Урок позаду?» — нешкідливо, але ефекту не має |

Крихта «Урок **01**» (два знаки, як у базі «Модуль 01») проти `<title>` «Урок **1**» — так і задумано,
не вирівнювати.
...
| - | ------------- | ------------------------------ |
| 1 | `1. Навіщо це` | яку задачу закриває — у сюжеті «Марципану» |
...
**Скелет іспиту (додано 2026-09-20 за звітом автора j23 — назви блоків уроку іспиту не
пасують, а чекер назв не перевіряє, лише число секцій ≥ 9).** Вісім блоків прози + секція
квізу, зразок форми — `modules/jira-23.html` (і `claude-code-23.html`): `1. Навіщо цей іспит` ·
`2. Як влаштований іспит` (таблиця механіки — **лише та, що є в `js/quiz.js`, `js/module.js`,
`js/progress.js`**: реальний бал на сервер не їде, тому «сервер перевірить» і «сертифікат лише
при 85 %» обіцяти не можна) · `3. Що саме перевіряється` (фази з `tracks[].title`) · `4. Весь курс
у N рядках` (назви уроків — дослівно з `modules[].title`) · `5. Перевір не лише себе, а й свій сайт` ·
`6. Типові граблі іспиту` · `7. Часті питання` · `8. Після іспиту` · `Іспит: 26 питань`
(`id="quiz"`). Відносно базового уроку змінюється **пʼять** місць, не чотири: додається крихта
шапки «Урок NN» → «Іспит». **Джерело правди для іспиту одне — чинний текст уроків** (не картки,
не докси): кожне питання має опору-цитату з уроку у звіті автора; правильна відповідь не стоїть
на ⚠, на написі вікна «за документацією», на числі чи даті сюжету; питань квізів уроків дослівно
не повторювати (перевірка — `difflib` проти всіх питань курсу); `explain` починається з «Урок N.».
...
- `.win__addr` — умовна адреса без домену, з `…` на початку; `aria-hidden`. ⚠ Шлях зі скелета —
  для **software**-спейсу (`…/jira/software/projects/KEY/boards/1`); для business-спейсів сюжету
  (`MY`, `MARK`, `REM`, `HR`) шлях не звірений — писати узагальнено `…/jira/…/MARK/board`, доки
  не буде знімка business-спейсу (`screens/21-…`, завдання наступної сесії).
- **Легенда — підписи, не речення:** пункт легенди ≤ ~80 знаків («Колонка = етап роботи»);
  пояснення на абзац — прозою під вікном. `check-lessons.py` шукає легенду одразу за `</figure>`
  у вікні 8 000 символів (до 2026-09-17 було 400 — давало хибне «пунктів легенди 0», коштувало
  часу трьом авторам).
...
<div class="win__portal">
  <h4 class="win__portal-title">Бухгалтерія «Марципану»</h4>
...
`win__link` · **`win__portal-intro`** (вступний текст порталу — поле Introduction text, автор j01) ·
**`win__topbar`** (глобальна верхня панель Jira: пошук · `+ Create` · `See plans` · дзвіночок — окремо
від `win__bar` спейсу; автор j02) · `win__mark` · `win__legend`.

⚠ Для дизайну: `win__table` на 4–6 колонок на 390 px потребує скролера всередині вікна (бриф,
поле 8) — у тимчасовій розмітці обгортки немає, автори не додають її самі.

**Конвенції, зафіксовані після j01–j03 (2026-09-17):** `div.win__page` дозволений у **всіх** станах як
контейнер «заголовок `<h4>` + вміст» головної області (у `list` варіант «домашня»: `win__page > h4 +
span.win__tabs + …`; дизайн може завести `win__head`); у стані **`settings` мітка ставиться на
`<dt>`** (назву поля), не на `<dd>`; одне вікно — одна `win__bar`, другий рядок вкладок живе в
`win__page`; іконки без текстових підписів (дзвіночок, довідка, шестірня) у вікно **не вигадувати** —
називати в підписі під вікном.
...
**Два прийоми, знайдені авторами 2026-09-17:** (а) індексні адреси `…/<product>/docs/` через
WebFetch віддають **404** — робочий вхід `…/<product>/resources/` або сторінка-листок через
WebSearch; частина сторінок віддає лише зміст — читати дочірню; (б) **багатоколонкові таблиці
планів WebFetch читає неправильно** (вигадує рядки, плутає колонки) — таблиці брати `curl`-ом
з Bash і розбирати `<tr>/<td>` (галочка = `<path d="M9.707 11.293…">`, прочерк = `-`).
