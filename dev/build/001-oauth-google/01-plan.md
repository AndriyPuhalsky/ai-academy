...
1. **[блокуюче — зовнішня дія, не код]** Провайдер Google у Supabase досі вимкнений.
   Перевірено щойно, `GET /auth/v1/settings` → `"google": false`. Крім самого провайдера
   потрібен ще й **allow-list редіректів**, інакше після Google людину викине на прод,
   а не на dev-превʼю, де її тестує QA. Повний перелік того, що має бути ввімкнено, —
   розділ 4.1. **Мій дефолт:** код пишеться повністю, бекендер і фронтендер не чекають;
   QA позначає наскрізний редірект як «не перевірено через зовнішній блокер».
...
5. **[неблокуюче]** «М'який» другий дотик (`mode: "soft"`) вимагає памʼятати, що людина вже
...
Перевірити живим редіректом сьогодні неможливо (провайдер вимкнений), тому — що відомо
напевно і що бекендер має зафіксувати дослівно:

| Джерело | Ключі | Де лежать |
| ------- | ----- | --------- |
| GoTrue (Supabase Auth) | `error`, `error_code`, `error_description` | **`location.search`** при PKCE-потоці, **`location.hash`** при implicit |
| GoTrue, успіх | `code` (+ `state`) | `location.search` при PKCE |

`supabase-js@2` у браузері за замовчуванням працює в **PKCE**, тобто очікуваний
формат — query-параметри. Але код із макета (`base.js:660–675`, `readOAuthError`) читає
**і `search`, і `hash`**, і три ключі одразу — це правильна оборонна форма, її й переносимо
без змін. Невідомі саме **літерали** `error_code` для двох випадків: «людина скасувала
на екрані Google» і «пошта вже зайнята паролем».

**Що робить бекендер, коли провайдер увімкнуть** (обовʼязково, у `02-backend/report.md`):
1. на dev-превʼю натиснути кнопку, на екрані згоди Google натиснути «Скасувати»;
2. **до** будь-якого кліку скопіювати `location.href` цілком і записати дослівно;
3. те саме для випадку «пошта вже має акаунт із паролем», якщо його вдасться відтворити
   (може не вдатися — див. розділ 8, ризик Р7);
4. звірити з мапінгом у розділі 5.4 і, якщо реальні коди не потрапляють у жоден регексп,
   **не чіпати регекспи, а дописати ще один** — дефолт `other` уже коректно ловить усе інше.
...
## 4. Що потрібно від власника (зовнішні дії, паралельно з роботою)

### 4.1 Увімкнути Google — чек-лист

**Google Cloud Console** → APIs & Services → Credentials → OAuth 2.0 Client ID (Web):
- Authorized JavaScript origins: `https://ai-academia.com.ua`,
  `https://ai-academy.andriy-puhalsky.workers.dev`,
  `https://dev-ai-academy.andriy-puhalsky.workers.dev`
- **Authorized redirect URI (рівно один, і саме цей):**
  `https://hpcyrnxschpxlrxudmqk.supabase.co/auth/v1/callback`

**Supabase Dashboard** → Authentication → Providers → Google: `Enabled`, Client ID + Secret.

**Supabase Dashboard** → Authentication → URL Configuration → **Redirect URLs** —
без цього кроку QA не зможе перевірити нічого:
```
...
http://localhost:8000/**
```
**Чому це критично:** якщо `redirectTo` не в allow-list, GoTrue мовчки підставляє
`Site URL`, тобто **прод**. Людина, яка тестує на dev-превʼю, після Google опиниться на
`ai-academia.com.ua` — і це виглядатиме як баг коду, яким не є.
...
### 6.4 Що можна робити, не чекаючи Google

Усе, крім самого редіректу:
- уся верстка, стани, доступність, рух, мобільний аркуш, скелетон;
- діалог імені й запис у `profiles` (працює вже сьогодні, вхід поштою);
- панелі помилок — їх можна викликати вручну, підставивши параметр у URL:
  `?error=access_denied&error_description=user+denied` → має відкритись модалка з панеллю
  «Вхід через Google скасовано» і **очиститись URL**. Це повноцінний тест логіки 5.7
  без живого Google;
- стан очікування й таймаут 8 с — перевіряються, якщо провайдер вимкнений:
  `signInWithOAuth` віддасть помилку → панель `open`.
...
14. **Очищення URL не зʼїдає чуже.** Відкрити
    ...
    `error` зник.
15. **Кнопка Google при вимкненому провайдері** дає панель `open` («Не вдалося відкрити
    Google»), а не мовчазне нічого і не зависання назавжди.
...
25. **Наскрізний Google-вхід** — коли власник увімкне провайдера: клік → екран Google →
    повернення на **ту саму сторінку** dev-превʼю → у шапці ім'я з Google ≤3 с →
    URL без `code` → консоль чиста. **До ввімкнення провайдера цей критерій позначається
    як «не перевірено через зовнішній блокер», і це не вважається дефектом.**
...
| Р2 | **`redirectTo` не в allow-list Supabase** → після Google людину викидає на прод замість dev-превʼю | QA «перевіряє» прод, думаючи що дивиться превʼю; можлива плутанина з реальними акаунтами | Dashboard → Authentication → URL Configuration, до першого тесту. Розділ 4.1 |
...
- наскрізний редірект до Google не працює, поки `external.google === false`
  (перевіряється командою з розділу 9) — це зовнішній блокер, позначити й іти далі;
