...
**Доповнення 2026-09-06 (006, коло 2): два нові факти про css/custom.css.**
- `.module-sidebar` — mobile-first: базове правило = шторка (`position:fixed`, `z-index:50`,
  над оверлеєм `.sidebar-overlay` z-45 і над шапкою уроку z-40), медіа-блок
  `@media (min-width:1024px)` = sticky-колонка і **з 006 явно скидає `z-index:10`**.
  Міняючи там будь-що, звіряти обидва списки властивостей: пропущений `z-index` і був
  коренем П-01.
- `.navprog-label` — власний клас мітки «Прогрес: » у `#navProgress` (секція L, поруч із
  `#navProgress[data-reserved]`). База = декларації Tailwind `sr-only`,
  `@media (min-width:640px)` = `not-sr-only`. Заведений замість пари утиліт
  `sr-only sm:not-sr-only`, бо клас приходив у DOM лише з JS і CDN генерував правило
  після вставки. Брейкпоінт 640 мусить збігатися з брейкпоінтом резерву
  (`--navprog-short-ch` / `--navprog-ch`) у тій самій секції.

**Доповнення 2026-09-07 (010, етапи 3–4): урок повністю переїхав на `css/components.css`.**
Стара мова (`prose-aia`, `callout*`, `example*`, `diagram*`, `lesson-no`, `module-sidebar`,
`snav-*`, `mnav*`, `quiz-*`, `tokens`) у `modules/*.html` більше не вживається — 0 входжень.
Що треба знати, беручи наступну задачу по уроку:
- **типографіка прози живе на ДВОХ рівнях**: `:is(.ds-prose, .ds-prose > section) > X`, бо
  у 56 із 56 уроків зміст лежить усередині `<section data-lesson>`. Третій рівень
  вкладеності не отримає нічого. `display: contents` на секції **вбиває скролспай**
  (`js/module.js` вішає `IntersectionObserver` саме на неї);
- **сітка прози**: текст — колонка `--w-narrow` (672), а `.ds-tbl__wrap`, `.ds-code`,
  `.ds-diag`, `.term` — `--w-prose` (768). Відкат до однієї колонки — атрибут
  `data-prose="wide"` на `<article>`, більше нічого;
- **`.ds-code__pre` оформлений лише як `.ds-code > …` і як `.ds-prose(> section) > …`.**
  `<pre>` на будь-якому іншому рівні (наприклад усередині `<li>`) лишиться голим —
  такий випадок у проді рівно один (`claude-code-22`), і він обгорнутий у `.ds-code`;
- **`<code>` у заголовку прози ламається**: `.ds-prose code` дає абсолютні `--fs-code`
  (13,52 px), а в проді було відносне `0.9em`. 125 заголовків у 18 файлах містять
  інлайновий `<code>`; найгірший випадок — `h1` 64 px проти 13,52. Правила в
  `components.css` **немає** — це відкритий пункт звіту Ф-Б;
- **`.ds-btn::after { content: "" }`** — порожній рядок, тому кнопка копіювання всередині
  `.term` (`term__copy ds-btn ds-btn--sm ds-btn--quiet`) правило «жодного псевдоелемента
  з `content` у `.term`» не порушує. А от `ds-btn--copy` туди додавати **не можна**:
  у нього `content: "✓"`.
