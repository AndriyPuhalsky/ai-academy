...
- [Пастки дистракторів іспиту](reference_exam_distractor_traps.md) — дистрактор на ⚠ уроку; «зокрема» в цитаті нічого не заперечує; маркером буває сполучник «бо»/«тому»
...
- [Перевірка фактів Jira (012)](reference_jira_docs_verification.md) — curl замість WebFetch; сторінки-хаби ≠ «факту немає»; ключ роботи не вічний; статуси з шаблону
- [Маркер стилю в квізі](reference_quiz_style_marker_blindspot.md) — check-quiz.py міряє лише довжину; тире/двокрапку «лише в правильному» ловити руками, правити тільки дистрактори
- [Шаблон спейсу → скільки статусів](reference_space_template_status_count.md) — облік задач 2 (To Do/Done), керування проєктом 3; ловить зламані кроки «Зроби сам»
- [Спільний скретчпад у хвилі](reference_shared_scratchpad_race.md) — сусід перезаписує завантажені докси; цитати звіряти у власній підпапці, інакше хибний «MISS»
- [Звертання до читача](reference_course_reader_address_forms.md) — парні форми «бачив(ла)»; урок лише в чоловічому роді (j11: 5 проти 0) — правка рецензента
- [Цитата без переказу](reference_quote_without_gloss.md) — англійська цитата без укр. перекладу = «непояснене слово»; перекладати може рецензент, числа-цитати не чіпати
- [Слова-двійники Jira](project_jira_term_twins.md) — workflow (j10) проти flow (j14), «фільтр» = збережений запит (j07); грепати у фазах 4–5
- [Твердження про саму довідку](reference_claims_about_the_docs.md) — «у доксах це звуть/скорочують», «кнопка живе на екрані Y», «є лише тоді, коли…» — грепати й звіряти зі screens/ (три хиби в j18)
- [Цитата обрізана перед уточненням](reference_quote_truncated_qualifier.md) — збіг посимвольно ≠ повна цитата; друкувати 200–300 знаків ПІСЛЯ збігу (j20: «…won't count towards your bill» без «(unless…)»)
- [Жива адреса й клієнтський рендер](reference_live_endpoint_and_client_rendered_pages.md) — 401 + `www-authenticate` = адреса жива (обидві MCP-адреси Atlassian); 200 з 32 символами тексту = малює браузер, а не «сторінки немає»
- [Реєстр тверджень без джерела](reference_unsourced_claims_registry.md) — «повідомляю, не правлю» у звіті ≠ закриття; найдорожче — «лише/завжди» + правильний варіант квіза; джерело часто в СУСІДНЬОМУ продукті доксів
