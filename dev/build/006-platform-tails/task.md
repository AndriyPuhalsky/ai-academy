...
## Рішення власника 2026-09-06 (після QA кола 2)

- **QA коло 2 пройдено** — звіт `04-qa/report-round2.md`.
- **Тестовий акаунт не чистимо.** На відміну від задачі 005 (де слід прибирала міграція `005-4`),
  тут міграція-чистка **не пишеться й не заводиться**: рядки тестового акаунта лишаються в
  продових `profiles` як є. Не пропонувати цю чистку повторно.
- Мерж у `main` — окреме рішення власника, поки його немає.
- Перед релізом лишається підняти версії трьох платформ і `site.updated`.
