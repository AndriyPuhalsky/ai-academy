...
(function () {
  ...
  var lit = { rect: null, headG: null, fullPath: null, len: 0 };
  ...
  function render() {
    ...
    lit.pulseRings = now.querySelectorAll(".rm-now__ring");
    return true;
  }
  ...
  function pointAtY(targetY) {
    if (!lit.len) return null;
    var lo = 0, hi = lit.len, mid, p;
    for (var i = 0; i < 22; i++) {
      mid = (lo + hi) / 2;
      p = lit.fullPath.getPointAtLength(mid);
      if (p.y < targetY) lo = mid; else hi = mid;
    }
    return lit.fullPath.getPointAtLength((lo + hi) / 2);
  }
  ...
  function start() {
    ...
    mm.add("(prefers-reduced-motion: no-preference)", function () {
      ...
      /* ---- M7 · ЄДИНИЙ безкінечний рух на сторінці ----
         Один твін на всі вузли «В роботі»: вони дихають в унісон,
      ...
      function bindPulse() {
        ...
        gsap.set(rings, { scale: 1, opacity: num("--opacity-pulse", 0.55) });
        pulse = gsap.to(rings, {
          scale: num("--scale-pulse", 1.9),
          opacity: 0,
          duration: sec("--dur-pulse", 1900),
          ease: ease("--e-breath", "sine.inOut"),
          repeat: -1
        });
      }
      bindPulse();
      document.addEventListener("rm:relayout", bindPulse);
      window.addEventListener("resize", bindPulse, { passive: true });
