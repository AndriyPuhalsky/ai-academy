...
// Схема успадкована з 009/02-references: дані окремо, збирач окремо —
// щоб падіння на етапі складання сторінки не забирало з собою виміри.
const fs = require('fs');
...
<article class="ref" id="${esc(r.id)}">
  ...
  <div class="body">
    <p><span class="lbl">Чому важливий</span>${md(r.why)}</p>
    ${r.motion ? `<p><span class="lbl lbl-m">Поведінка</span>${md(r.motion)}</p>` : ''}
...
body{margin:0;background:var(--n1);color:var(--n12);font:16px/1.7 var(--f-s);-webkit-font-smoothing:antialiased;-webkit-tap-highlight-color:transparent}
...
.shots{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(230px,100%),1fr));gap:14px;margin-top:16px}
figure{margin:0}
...
table.data{width:100%;border-collapse:collapse;font-size:13px;margin:0 0 24px;display:block;overflow-x:auto;overscroll-behavior-x:contain}
...
code.m{background:transparent;padding:0}
...
.labs{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(320px,100%),1fr));gap:16px}
.labcard{background:var(--n2);border:1px solid var(--n6);border-radius:14px;padding:18px 20px}
.labcard h3{margin:0 0 8px;font:600 16px/1.4 var(--f-s)}
.labcard p{margin:0;color:var(--n11);font-size:14px}
footer{margin-top:80px;padding-top:24px;border-top:1px solid var(--n6);color:var(--n10);font-size:13px}
@media (max-width:760px){.wrap{padding:32px 18px 80px}h1{font-size:32px}nav.toc ol{columns:1}.ref>header{flex-wrap:wrap}.feas{max-width:none}.shots{grid-template-columns:1fr}}
...
console.log('index.html: ' + (html.length / 1024).toFixed(1) + ' KB, секцій ' + D.sections.length +
  ', карток ' + D.sections.reduce((n, s) => n + (s.refs ? s.refs.length : 0), 0));
