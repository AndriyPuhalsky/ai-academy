/* ============================================================================
   010 · js/roadmap-render.js — КОПІЯ ПРОДУ. ЛОГІКА НЕ ЗМІНЕНА.
   ----------------------------------------------------------------------------
   Змінено дві речі, обидві названі на місці:
     1. getPropertyValue("--sk-rows") → getPropertyValue("--rm-sk-rows")
     2. додано перемикання data-course разом із data-brand за ?from= (§5.2)
   Причина: токен переїхав із глобального :root у css/roadmap.css і отримав
   префікс сторінки. Без цієї заміни parseInt дає NaN і скелетон мовчки
   малює 6 рядків із fallback — тобто розсинхрон не видно взагалі.

   ⚠ ЦЕ 37-й ТОКЕН, ЯКИЙ ЧИТАЄ JS. Спека §12 П-06 нарахувала 36 і перелічила
   лише ті, що читає roadmap-motion.js. render.js читає ще два:
       --sk-rows        (рядок 199) — перейменовується
       --delay-skeleton (рядок ~92) — імʼя збігається із системним, не чіпаємо
   Перейменовувати треба ТРИ файли одним комітом, а не два.

   ⚠ ДВІ ЛАТЕНТНІ ПАСТКИ ЦЬОГО ФАЙЛУ НЕ ЧІПАЛИСЬ (пряма заборона брифа §7):
     1. згортання квартальних груп іде за індексом (gi >= 2) — при появі
        третього кварталу згорнеться найсвіжіший, а не найстаріший;
     2. гілка зі станом dropped сортує «Зроблено» у зворотному порядку.
   Обидві не видно на поточних даних. НЕ «лагодити по дорозі».
   ========================================================================== */
...
(function () {
  ...
  function build(data) {
    ...
    /* ⚠ ДОДАНО 010 (два рядки). §5.2 спеки: «roadmap.html — data-course="academy"
       за замовчуванням, і js/roadmap-render.js перемикає його разом із брендом
       за ?from= (той самий механізм, що вже перемикає data-brand)». У проді
       цього рядка немає, бо до 010 акцент був один на весь сайт. Без нього
       людина, яка прийшла з AI Architect, бачить бренд «AI Architect» і
       ШАФРАНОВИЙ акцент Академії — тобто слот акценту курсу на єдиній спільній
       сторінці сайту не працює саме там, де він найпотрібніший. */
  ...
  function section(state, items, c, data) {
    ...
    if (state === "done") {
      ...
      //
      // ⚠ 2026-09-04: було `gi >= 2`, і це згортало третю групу ВІД ПОЧАТКУ.
      // Порядок пунктів у roadmap.json — від найстарішого до найновішого
      // (рішення власника, див. _note у файлі), а groupByQuarter його зберігає
      // і не сортує. Отже старий вираз ховав НАЙСВІЖІШИЙ квартал замість
      // найстарішого — рівно навпаки до коментаря над ним. На двох групах це
      // не було видно; третя зʼявиться, щойно щось із «В роботі» стане `done`.
      // Рахуємо від кінця: відкритими лишаються LAST_OPEN найсвіжіших.
      var LAST_OPEN = 2;
