/* ============================================================================
   010 · js/roadmap-motion.js — ХОРЕОГРАФІЯ, ПЕРЕВЕДЕНА НА СИСТЕМУ 009
   ----------------------------------------------------------------------------
   ЛОГІКА НЕ ПЕРЕПИСАНА (§7.6). Змінено рівно три речі, і кожна названа:

   1. 36 ІМЕН ТОКЕНІВ — синхронно з css/roadmap.css. Повна таблиця в
      REPORT.md. Розсинхрон тут НЕ ВИДНО на екрані: num()/ms() тихо беруть
      fallback-число, і роадмап анімується старими зашитими значеннями (П-06).
      Чотири імені зведені до системних, бо система вже має цю роль:
        --scrub-trail, --scrub-outro → --scrub   (0.8, збіг)
        --dur-pulse    → --dur-loop-pulse        (1900, збіг байт у байт)
        --dur-pulse-in → --dur-enter             (180, збіг байт у байт)
        --dur-count    → --dur-count             (900 замість 800)
        --e-slow, --e-count → --e-out            (кривих у системі рівно пʼять,
                                                  шоста заборонена)

   2. motionOn() + staticState() — П-27. gsap.matchMedia() бачить лише
      СИСТЕМНУ настройку; пресет data-motion="calm" і перемикач html.rm
      ставлять --motion: 0, і без цієї перевірки роадмап лишався б живим
      саме там, де рух вимкнули.

   3. root.classList.add("rm-armed") на першому рядку observeRows() —
      перевертає полярність приховування (див. коментар на місці), плюс дві
      страховки того самого дефекту, які система вже має для [data-reveal]:
      що вже у вʼюпорті — не ховається, і на visibilitychange спостереження
      переоголошується.

   Що НЕ змінилось: два ScrollTrigger'и на сторінку, збереження фази пульсу
   при rebind, порядок render → wireHover → observeRows → start, і фікс
   кола 1 задачі 003 (перенесення прогресу змійки на щойно створені вузли).
   ⚠ Останній — це рівно той дефект D-01, який лишився в макеті 003 і
   виправлений тільки в коді сайту. Ця копія зроблена З КОДУ САЙТУ.
   ========================================================================== */
...
function buildLUT() {
  ...
  function observeRows(scope) {
    /* ⚠ ДОДАНО 010 (один рядок, логіки не змінює).
       ...
       Тепер ховає лише клас, який ставиться ТУТ, за крок до o.observe():
