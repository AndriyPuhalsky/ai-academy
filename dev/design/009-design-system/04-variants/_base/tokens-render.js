...
(function (global) {
  ...
  /* ⚠ ФІКС АГЕНТА №4 (D-03). Було три залежності від асинхронних колбеків,
     і всі три ламались у відтвореному сценарії «відкрив → Esc → відкрив»:
       (1) data-open вмикався всередині requestAnimationFrame. Заміряно: у
           третьому колі кадр не прийшов узагалі — діалог лишався hidden=false
           з opacity 0, тобто невидимий, але з пасткою фокуса. Це той самий
           клас дефекту, що 003 D-01 і 005 Б-01, лише в діалозі. Лікується
           примусовим reflow — прийом, який уже вживає сам motion.js;
       (2) setTimeout закриття ніхто не скасовував: у другому колі він гасив
           щойно відкритий діалог (виміряно hidden=true через 600 мс після
           відкриття);
       (3) тривалість читалась зі СКЛАДЕНОГО токена --dur-exit — рівно те, що
           доктрина системи забороняє JS робити. Тепер через AIA.motion.dur(),
           який читає сире --p-dur-exit і множить сам.
     Плюс критерій 22: блокування скролу (клас html.ds-lock). */
  var lastFocus = null, dlgTimer = null;
  function exitMs() {
    var M = global.AIA && global.AIA.motion;
    return M ? M.dur("exit") * 1000 : 0;
  }
  function openDlg() {
    ...
    d.hidden = false;
    void d.offsetWidth;                      /* детерміновано замість rAF */
