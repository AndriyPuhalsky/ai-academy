/* ============================================================
   001 — Вхід через Google · ЛОГІКА Й РУХ
   ------------------------------------------------------------
...
(function () {
          ...
          '<div class="aia-dim" id="aiaEmailPath">' +
            ...
            '<div class="aia-stack" id="aiaForm" role="tabpanel" aria-labelledby="aiaTab' + (tab === "register" ? "Register" : "Login") + '">' +
              '<div class="aia-collapse' + (tab === "register" ? " is-open" : "") + '" id="aiaNameWrap">' +
                "<div>" +
                  '<input id="aiaName" class="aia-input" type="text" placeholder="' + esc(T.namePh) + '" autocomplete="name" maxlength="100" />' +
  ...
  function skeletonHtml(isUser, narrow) {
    ...
    return narrow
      ? '<span class="sk sk--avatar" aria-hidden="true"></span>'
      : '<span class="sk sk--user" aria-hidden="true"></span>';
  }
  ...
  function onComplete(e) {
    ...
    // Тригер: клік переводить курс у n/n. Не «квіз», не сторінка сертифіката.
    if (demo.modProgress === 11) {
      openNameDialog(demo.nameTouched ? "soft" : "last", btn, function () {
        demo.modProgress = 12;
        renderSite();
        flash("Ім'я підтверджено → тільки тепер викликано <b>submit_quiz</b>. " +
              "Лічильник у шапці: 12/12.");
      });
      return;
    }
    demo.modProgress++;
  ...
  var SCENES = {
    a1: {
      ...
      note: "Базовий кадр. Порядок блоків — R01 (Cal.com): <b>кнопка провайдера → хайрлайн «або» → форма → повноширинна головна кнопка внизу</b>. " +
            "Соц-блок стоїть <b>вище</b> табів, тому він не належить жодному з них. У модалці рівно <b>два</b> повноширинні кольорові контроли.",
      actions: [
        ["Дублювати кнопку в #aiaSocial", function () {
          var s = stage.querySelector("#aiaSocial");
          if (!s) return;
          var wrap = document.createElement("div");
          wrap.innerHTML = buildProviderButton("aiaLinkedIn", "light", "Продовжити з LinkedIn");
          s.appendChild(wrap.firstElementChild);
          flash("Друга дитина додана наживо. Змінилась <b>тільки висота картки</b>: ширина, відступи й порядок решти елементів не зрушили (критерій 4).");
        }],
    ...
    a2: {
      ...
      page: "home", width: 1280, session: "guest",
      note: "Найдовша з трьох нотаток сайту (62 символи, <code>js/progress.js:33</code>). " +
            "Для більшості учнів це <b>перший</b> показ модалки взагалі, тому нотатка проєктується як норма, а не як рідкість.",
    ...
    a3: {
      ...
      page: "home", width: 1280, session: "guest",
      note: "Ефект 5: поле імені й підказка розкриваються через <code>grid-template-rows: 0fr → 1fr</code>, 200 мс. " +
            "Складений стан = рівно 0 разом із відступом, тому висота картки не смикається під курсором. " +
            "Підказка тепер <b>muted (6.63)</b>, а не faint (3.57 — провал AA).",
    ...
    a4: {
      ...
      page: "home", width: 1280, session: "guest",
      note: "Ефекти 9 + 10. Смуга 2 px <b>clay</b> по верхній межі картки; решта діалогу гасне до 0.45 і не приймає кліків; " +
            "кнопка Google лишається на повній непрозорості. <b>Текст на кнопці не змінився</b> — це порушило б дозволені рядки Google. " +
            "Статус «Відкриваємо Google…» — окремий елемент із <code>role=\"status\"</code>.",
    ...
    a5: {
      ...
      page: "home", width: 1280, session: "guest",
      note: "Найгірший реальний стан — «вічно крутиться». Через 8 с смуга зупиняється, <code>aria-busy</code> знімається, " +
            "кнопка знову активна, з'являється панель. Нижче можна не чекати вісім секунд.",
      actions: [
    ...
    a6: {
      ...
      run: function () {
        ...
        setNote(
          "Цей стан приходить <b>після повного перезавантаження сторінки</b>, коли модалки вже немає — " +
          "тому «показати текст у #aiaError» не працює, його в цю мить не існує. Сторінка <b>сама</b> відкриває модалку за ознакою в URL і одразу URL чистить.<br>" +
          "URL до: <code>" + esc(before.split("/").pop()) + "</code><br>" +
          "URL після <code>history.replaceState</code>: <code>" + esc(location.href.split("/").pop() || "/") + "</code> — F5 помилку не повторить (критерій 16).<br>" +
          "Помилка — <b>панель</b>, а не рядок (прийом R03): іконка + заголовок + причина + що робити + кнопка дії, над формою email. Форма під нею лишається повністю робочою."
        );
      }
    },
    ...
    a7: {
      ...
      page: "home", width: 1280, session: "guest",
      note: "Тупик перетворено на один крок: таб «Вхід» активний, поле email <b>уже заповнене</b> тим, що повернув провайдер, фокус — у полі пароля. " +
            "Кнопки дії в панелі немає навмисно: дія тут не «спробувати ще раз», а «ввести пароль нижче».",
      run: function () {
        renderSite();
    ...
    a8: {
      ...
      page: "home", width: 1280, session: "guest",
      note: "Єдиний зі сценаріїв, який приходить <b>без перезавантаження</b> — сторінка ще наша. Тому панель просто з'являється в уже відкритій модалці (ефект 11, 140 мс).",
      run: function () {
        renderSite();
        var el = openAuth({});
    ...
    b1: {
      ...
      page: "home", width: 1280, session: "guest",
      note: "Слот <code>#aiaAuth</code> існує з першого кадру й має <code>min-height: 34px</code>, тому підміна не зсуває нічого в шапці. " +
            "Розмір плитки — <b>75×32</b>, це виміряна кнопка «Увійти» (R07). Shimmer їде <code>transform: translateX()</code>, не <code>background-position</code> — чистий композитинг.",
      actions: [["Програти ще раз", function () { playSkeleton(2600); }]],
      run: function () { playSkeleton(2600); }
    },

    b2: {
      group: "Шапка", label: "Скелетон: залогінений (десктоп)",
      page: "home", width: 1280, session: "user",
      note: "Форма інша: <b>307×34</b> замість 75×32. Різниця на десктопі — <b>232 px</b>, тому вгадувати обов'язково; " +
            "інакше шапка стрибне. Форму беремо не з хардкоду, а з <code>sb.auth.storageKey</code> у localStorage; ключа немає → гість.",
      actions: [["Програти ще раз", function () { playSkeleton(2600); }]],
      run: function () { playSkeleton(2600); }
    },
    ...
    b3: {
      ...
      page: "home", width: 1280, session: "user",
      note: "Crossfade 180 мс. Дивись на сусідні елементи шапки — лічильник прогресу й бургер не рухаються (CLS = 0, критерій 25).",
      actions: [
    ...
    b4: {
      ...
      page: "home", width: 1280, session: "user",
      note: "Поріг 150 мс. Якщо сесія вирішилась швидше — <b>скелетон не показується взагалі</b>, жодного блимання (критерій 26). Ось так це виглядає на 80 мс.",
      actions: [
    ...
    c1: {
      ...
      page: "home", width: 1280, session: "user",
      note: "Вхід — ім'я в шапці, яке зі <code>&lt;span&gt;</code> стало <code>&lt;button&gt;</code> з <code>aria-label</code>. " +
            ...
            "Вона виходить за паддінг картки <b>в обріз</b> — це навмисно: панель із рамкою сказала б «ще один блок UI», а обріз каже «це папір». Спробуй набрати ім'я.",
      run: function () {
        renderSite();
        var b = document.getElementById("aiaNameBtn");
        openNameDialog("permanent", b);
      }
    },
    ...
    c2: {
      ...
      note: "Тригер — клік по <code>#completeBtn</code> на сторінці модуля у стані, коли він переводить курс у <b>n/n</b>. " +
            "Не квіз (<code>js/quiz.js</code> серверу нічого не шле) і не сторінка сертифіката (там уже пізно). " +
            "Підложка не закриває, потрібен один із двох явних кліків. <b>Escape працює</b> і означає «скасувати завершення модуля» — " +
            "<code>submit_quiz</code> не викликається (критерій 29). Спробуй обидва шляхи.",
      actions: [
        ["Модуль 5/12 — дотику немає", function () { demo.modProgress = 5; demo.nameTouched = false; renderSite(); }],
        ["Модуль 11/12 — дотик є", function () { demo.modProgress = 11; demo.nameTouched = false; renderSite(); }]
      ],
      run: function () { demo.modProgress = 11; demo.nameTouched = false; renderSite(); }
    },

    c3: {
      group: "Ім'я для сертифіката", label: "Другий раз, м'якше",
      page: "module", width: 1280, session: "user",
      note: "Курсів два (12 і 22 модулі), тому дотик спрацьовує до двох разів. Удруге — той самий діалог, але <b>без режиму редагування за замовчуванням</b>: " +
            "заголовок, прев'ю, «Так, усе вірно». «Змінити ім'я» лишається, просто тихішим.",
      run: function () {
        demo.modProgress = 11; demo.nameTouched = true; demo.name = "Олена Ковальчук";
        renderSite();
        openNameDialog("soft", document.getElementById("completeBtn"), function () {
          demo.modProgress = 12; renderSite();
        });
      }
    },
    ...
    c4: {
      ...
      page: "module", width: 1280, session: "user",
      note: "Евристика, перевірювана без вгадування: містить <code>@</code> · одне слово · увесь нижній регістр · коротше 4 символів. " +
            "Тут спрацювали дві умови одразу («olena p» — нижній регістр і фактично ініціал).<br>" +
            "Попередження стоїть <b>на темному боці</b>, а не обведенням смужки: clay на кремовому дає 2.85:1 і не пройшов би 3:1, " +
            "тоді як clay на surface = 5.49. Це нічого не блокує й не міняє кнопок.",
      run: function () {
        demo.modProgress = 11; demo.nameTouched = false; demo.name = "olena p";
        renderSite();
        openNameDialog("last", document.getElementById("completeBtn"), function () {
          demo.modProgress = 12; renderSite();
        });
      }
    },
    ...
    d1: {
      ...
      note: "Сьогодні на &lt;640 залогінений бачить у шапці <b>лише «Вийти»</b>: ні імені, ні шляху до сертифікатів. " +
            "Три елементи згорнуто в кнопку-кружечок 36×36 з ініціалом → аркуш знизу.<br>" +
            "Рух — крива й тривалість Vaul (R05): 400 мс <code>cubic-bezier(0.32, 0.72, 0, 1)</code>, підложка й аркуш <b>однією тривалістю</b>, без стагера. Перетягування пальцем свідомо не робимо.<br>" +
            "<b>Межа обсягу тримається:</b> рядок «Ім'я для сертифіката» тут нічого не редагує — він відкриває той самий діалог, що й на десктопі. Це нова точка входу, а не нова поверхня.",
      run: function () {
        renderSite();
        var av = document.getElementById("aiaAvatar");
        if (av) openSheet(av);
    ...
    d2: {
      ...
      note: "Найвужчий випадок: ширина контенту картки = 240 px. Виміряний запас під підписом «Продовжити з Google» — <b>+37 px</b>, " +
            "тому падіння картки з <code>p-6</code> на <code>p-5</code>, яке передбачала спека, <b>не потрібне</b>. " +
            "«Продовжити з LinkedIn» у той самий бюджет теж влазить (+25 px). Точні числа — у консолі браузера.",
      run: function () { renderSite(); openAuth({ note: T.notes.progress }); }
    },

    e1: {
      group: "Окремо", label: "Кнопка Google: світла vs темна",
      page: "home", width: 1280, session: "guest",
      note: "",
      run: function () { renderCompare(); }
    },

    e2: {
      group: "Окремо", label: "prefers-reduced-motion",
      page: "home", width: 1280, session: "guest", rm: true,
      note: "Окремий кадр, а не побічний ефект. Усі 12 ефектів вимкнено: модалка з'являється миттєво й повністю, плашка таба перестрибує, " +
            "shimmer стає статичною плиткою <code>raised</code>, смуга процесу — статичною <b>clay</b> на 40 %. Нічого не зникає й нічого не ламається.<br>" +
            "У <code>tokens.css</code> це зроблено одним місцем: усі <code>--dur-*</code> перевизначені в 0.001 мс + два точкові правила для loop-анімацій.",
      actions: [
  ...
  function renderCompare() {
    ...
    b += '<p class="site__eyebrow">// критерій приймання 10</p>';
    b += '<h1 class="site__h1" style="font-size:30px">Обидва офіційні варіанти на справжньому <code style="font-family:var(--f-mono);font-size:0.7em;color:var(--c-sand)">bg-surface</code></h1>';
    b += '<p class="site__lead" style="font-size:15px">Однакова ширина картки (384 px), однаковий фон <code style="font-family:var(--f-mono);color:var(--c-sand)">#1E1B18</code>, живий інлайновий логотип. Обрано <b style="color:var(--c-sand)">світлий</b> — рішення власника §7.2.</p>';
    b += '<div class="gcompare-grid" style="margin-top:var(--s-8)">';

    b += '<div class="aia-card" style="animation:none">' +
           '<div class="gcompare"><div class="gcompare__item">' +
             '<p class="gcompare__label">Light — обрано</p>' +
             buildProviderButton("cmpLight", "light", T.google) +
             '<p class="gcompare__num">заливка <b>#FFFFFF</b> · обведення <b>#747775</b> 1 px inside · текст <b>#1F1F1F</b><br>' +
             'заливка до surface: <b>17.14:1</b> · обведення до surface: <b>3.79:1</b> ✓</p>' +
           "</div></div></div>";
    ...
    b += '<div class="aia-card" style="animation:none">' +
           '<div class="gcompare"><div class="gcompare__item">' +
             ...
             '<p class="gcompare__num">заливка <b>#131314</b> · обведення <b>#8E918F</b> 1 px inside · текст <b>#E3E3E3</b><br>' +
             '<span class="gcompare__warn">заливка до surface: 1.08:1 — плитка невидима</span>. ' +
             'Тримається виключно на обведенні (5.38:1), тому обведення тут <b>незнімне</b> (критерій 11).</p>' +
           "</div></div></div>";

    b += '<div class="aia-card is-wide" style="animation:none;padding:var(--s-4)">' +
           '<p class="gcompare__num" style="margin:0">Геометрія обох: висота <b>40 px</b> (не масштабуємо до 44 — рішення §7.2), радіус <b>4 px</b>, ' +
           'паддінги <b>12 / 10 / 12</b>, логотип <b>18×18</b>, підпис <b>14/20, ls 0.25 px</b>. ' +
           'Підпис — офіційний рядок Google для локалі uk (<code>continue_with</code>), не наш переклад.</p></div>';

    b += "</div></div>";
    host.insertAdjacentHTML("beforeend", b);
    setNote(
      "Обидва офіційні варіанти поруч, на справжньому фоні картки. Темний показано <b>з обов'язковим обведенням</b> — без нього його заливка дає 1.08:1 до нашого <code>surface</code> і кнопка просто зникає.<br>" +
      "Звірка з першоджерелом <code>developers.google.com/identity/branding-guidelines</code> — повторена мною <b>2026-08-22</b>. " +
      "Відхилення від guidelines рівно одне: гарнітура підпису (IBM Plex Sans Medium замість Google Sans) — причини в NOTES.md §2.3."
    );
  }
  ...
  function buildRail() {
    ...
    var h = '<p class="rig__brand">001 · вхід через Google</p>' +
            '<p class="rig__sub">Живий макет. Усе клікабельне: модалка, таби, поля, аркуш, дотик з іменем.</p>';
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("toolbar").addEventListener("click", function (e) {
      ...
      var kv = b.getAttribute("data-set").split(":");
      var v = kv[1];
      demo[kv[0]] = (v === "true") ? true : (v === "false") ? false : (isNaN(+v) ? v : +v);
      if (kv[0] === "rm") applyRm();
      syncToolbar();
      wipeDialogs();
